...
@ HyperIMU - ianovir
@ Date:Thu Mar 21 14:54:29 EDT 2024, Sampling Rate:20ms


LeftArm info likewise:
@ HyperIMU - ianovir
@ Date:Thu Mar 21 14:54:28 EDT 2024, Sampling Rate:20ms
...
LEFTARM = read.csv(here(path, "LEFTARM_TEST.csv"))

#Import all RIGHTLEGredo data
RIGHTLEG = read.csv(here(path, "RIGHTLEG_TEST.csv"))
```
...
```{r}
#Unlike with the training dataset where I recorded each move one at a time consecutively without repeating any move, the dance routine I have made repeats many moves. Therefore I must structure this very different. Unfortunately, we can't have the row name corresponding to the dance number or dance name since that wil result in duplicate row names.. which gives me error. Boohoo.

dance = data.frame(row.names = c(0:41))

dance$order = c("Salsa", "LeftToRight", "Salsa", "Turns", "Salsa", "LeftToRight", "Salsa", "Turns", "KneeUpSideToSide", "KneeUp", "FourCorners", "OneTwoStep", "SwayNoodleArms", "SwayBlockArms", "SideJumpingJacks", "ThtBlondeGirl", "SideJumpingJacks", "ThtBlondeGirl", "SideJumpingJacks", "ThtBlondeGirl", "HoppityHop", "OneTwoThreeUp", "HoppityHop","OneTwoThree", "OneTwoStepCombo", "SwayNoodleArms", "SwayBlockArms", "BabySteps", "SlidyLeg", "BabySteps", "SlidyLeg", "BabySteps", "SlidyLeg", "BabySteps", "SlidyLeg", "HitIt", "LegLegUpDown", "HitIt", "OppaGangnamStyle", "HitIt", "LegLegUpDown", "HitIt")

#Corresponding number of each dance, it's the same as in the trainingset file
dance$number = c(2,13,2,4,2,13,2,4,5,11,8,7,9,1,3,15,3,15,3,15,0,18,0,16,17,9,1,12,20,12,20,12,20,12,20,19,14,19,10,19,14,19)
          

#Now, we must do the start and stop times just like with the training data
dance$starttime = c(149, 151, 153, 154, 157, 159, 161, 163, 165, 171, 179, 190, 196, 199, 205, 207, 210, 212, 215, 217, 220, 222, 226, 227.5, 232.5, 246, 253, 259.5, 262, 263, 265.5, 267.5, 269, 270.5, 272.5, 274.5, 278, 283, 286.5, 292, 296, 301)

dance$stoptime = c(151, 153, 154, 157, 159, 161, 163, 165, 171, 179, 190, 196, 199, 205, 207, 210, 212, 215, 217, 220, 222, 226, 227.5, 232.5, 246, 253, 259.5, 262, 263, 265.5, 267.5, 269, 270.5, 272.5, 274.5, 278, 283, 286.5, 292, 296, 301, 303.5)

#My numbers are slightly off depending on rightleg vs leftarm. I count both to 353.5 seconds.. Let's fix that:
dance$starttime = dance$starttime + (((355.049 + 355.14)/2) - 353.5)
dance$stoptime = dance$stoptime + (((355.049 + 355.14)/2) - 353.5)


#Now we will actually assign the numbers to each data entry..

#This beyond gorgeous loop simply assigns every entry that has a seconds_elapsed value within the starttime-stoptime parameters its corresponding dance label. I use my dancebynames and dancebynumbers dataframes to iterate through every dance move. It is much more effective than doing every single dataframe for every dance move. This would be 2*41 or 82 explicit assignments. Instead every dataframe appears once and the assignment is done through reference variables. Much better.

for (x in 0:41){
yeah = as.character(x)

LEFTARM$dance[(LEFTARM$seconds_elapsed < dance[yeah,]$stoptime) & (LEFTARM$seconds_elapsed >= dance[yeah,]$starttime)] = dance[yeah,]$number

RIGHTLEG$dance[(RIGHTLEG$seconds_elapsed < dance[yeah,]$stoptime) & (RIGHTLEG$seconds_elapsed >= dance[yeah,]$starttime)] = dance[yeah,]$number
}
...
```{r}
#Subset all values with no dancing.

LEFTARM = LEFTARM[(is.na(LEFTARM$dance) > 0) == FALSE,]

RIGHTLEG = RIGHTLEG[(is.na(RIGHTLEG$dance) > 0) == FALSE,]
...
```{r}
# Define parameters
#We have a total of 154 seconds (149 to 303.5) captured. Both were done at a rate of 20ms or 50Hz, yet we have roughly 28 hertz for the arm (4314/154) and 40 Hz for the leg (6193/154). For this reason, we will have slightly different filters for each
...
```{r}
write.table(dance, here(path, "SavedFrames/TESTING_danceorder.txt"))
write.table(LEFTARM, here(path, "SavedFrames/LEFTARM_testingsubset.txt"))
write.table(RIGHTLEG, here(path, "SavedFrames/RIGHTLEG_testingsubset.txt"))
```
